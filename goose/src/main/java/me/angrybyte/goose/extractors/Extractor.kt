...
/**
 * Created by IntelliJ IDEA. User: robbie Date: 5/19/11 Time: 2:45 PM
 */

/**
 * Encapsulates the process of extracting some type `T` from an article
 *
 * @param <T> the type of [Object] the implementing class will return
</T> */
interface Extractor<T> {

    /**
     * Given the specified [Element], extract @param <T>
     *
     * @param rootElement passed in from the [ContentExtractor] after the article has been parsed
     * @return an instance of type `T`
    </T> */
    fun extract(rootElement: Element): T
}
